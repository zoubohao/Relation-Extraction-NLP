...
### 将上一步的输出作为输入继续处理
train = open('D:\MyProgram\Data\semeval_task9_train_pair\\test0.txt','r')
fw = open('D:\MyProgram\Data\semeval_task9_train_pair\\test1.txt','w')

def preProcess(asent):
	asent = asent.replace(',',' ,')
	asent = asent.replace('-',' ')
	asent = asent.replace('/',' / ')
	asent = re.sub('\d', "dg", asent)
	asent = ' '.join(word_tokenize(asent))
	return asent
...
for s in train.read().strip().split('\n\n'):
	...
	for p in pair:
		...
		if (d1_spain.find(';') == -1) and (d2_spain.find(';') == -1):
			...
			d2e = int(d2e)
			d2s = int(d2s)
		other = set(e_dict) - {d1_spain, d2_spain}
		aaa = 'a' * (d1e - d1s + 1)
